...
<div class="box-body">
    ...
    <?= $form->field($model, 'category_id')->dropdownList(ArticleCategory::instance()->getListData('id', 'category_name'), ['prompt' => '', 'data-placeholder' => '请选择', 'class' => 'form-control select2', 'style' => 'width:100%'])->label('父级标题') ?>
    ...
    <?= $form->field($model, 'content')->widget(backend\widgets\CKEditor::class) ?>
    <?= $form->field($model, 'views')->textInput(['type' => 'number']) ?>
    <?= $form->field($model, 'sort')->textInput(['type' => 'number', 'min' => 0, 'max' => 1000]) ?>
    <?= $form->field($model, 'is_link')->radioList(['1' => '是', '0' => '否'], ['itemOptions' => ['class' => 'minimal'],  'item' => function ($index, $label, $name, $checked, $value) {
        return '<label class="radio-inline">' .
            Html::radio($name, $checked, ['value' => $value]) . $label .
            '</label>';
    }]) ?>
    <?= $form->field($model, 'link_url')->textInput(['maxlength' => true]) ?>
    <?= $form->field($model, 'is_hot')->radiolist(['1' => '是', '0' => '否'], ['itemOptions' => ['class' => 'minimal'],  'item' => function ($index, $label, $name, $checked, $value) {
        return '<label class="radio-inline">' .
            Html::radio($name, $checked, ['value' => $value]) . $label .
            '</label>';
    }]) ?>
